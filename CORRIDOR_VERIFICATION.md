...
**Reference amount: €1,000 sent, EUR → TRY, quoted 2026-08-16.** Figures below are per-channel; a genuinely simultaneous multi-provider quote pull (same minute, same amount, screenshotted) is still the recommended step before this is used as audited external evidence — see the caveats in each row.

| Channel | Exchange rate applied | Fee | FX markup (embedded, vs. mid-market) | Recipient receives | Settlement time | Source |
|---|---|---|---|---|---|---|
| **Wise** | 1 EUR = 55.4001 TRY (Wise states this *is* the mid-market rate — no separate markup layer) | €6.91 (0.691%, transparent, itemized before sending) | ~0% — Wise's stated model applies the mid-market rate directly, with only the disclosed fee deducted | **55,017.29 TRY** | Minutes to 1 business day (bank-funded) | Live-fetched from `wise.com/us/compare/xe-eur-to-try` (real-time, per-amount calculator), 2026-08-16 |
| **Western Union** | Not obtainable live — WU's currency-converter page renders its rate via client-side JavaScript that automated fetch tooling could not execute in this session (two direct fetch attempts timed out) | **Estimated** €0–15 for an online, bank/debit-funded transfer to a bank account (cash pickup and card funding typically cost more) | **Estimated** 1.2%–4% above mid-market, corridor- and payout-method-dependent; cash pickup trends toward the higher end | **Estimated ~52,350–54,735 TRY** (i.e., roughly €12–55 all-in cost on €1,000, applied against the same 55.4001 mid-market rate above — computed from the estimated fee + markup ranges, not from a fetched WU quote) | Minutes (cash pickup) to 1 business day (bank deposit) | Published fee/markup comparisons — `remitroutes.com/blog/western-union-fees-2026-all-methods` and corroborating range from general market comparisons, both accessed 2026-08-16. **Not a live Western Union quote** — flagged explicitly rather than presented as one. |
| **Traditional bank wire (SWIFT)** | Not obtainable live — no bank publishes a single standard rate; varies by originating bank | $35–50 (~€30–45) outgoing wire fee, **plus** typically 1–3 correspondent "lifting fees" of $15–50 (~€13–45) each on a EUR→TRY route (Turkey is outside SEPA, so intermediary routing is common), **plus** a receiving-bank fee of ~$15 (~€13) | 1%–3% typical, "some go higher" to 3%–5% at less competitive banks — rarely itemized separately from the quoted rate, which is the core transparency complaint about this channel | **Estimated ~47,644–51,522 TRY**, i.e. roughly **€70–140 all-in cost on €1,000** (7%–14%) once fee + markup + intermediary deductions are combined against the same 55.4001 mid-market rate — the widest and least predictable range of the three incumbents | 1–5 business days | `corpay.com/resources/blog/wire-transfer-fees`, accessed 2026-08-16 |
| **Ferry (target model)** | Firm, anchor-quoted rate via SEP-38, shown as an exact net `buy_amount` **before** the sender confirms — not an estimate | Anchor-set, itemized in the SEP-38 fee breakdown | Whatever the contracted anchor sets — Ferry itself adds none (non-custodial, no Ferry-side spread) | Depends entirely on the contracted pilot anchor's fee schedule — **not yet established, see §1–§2 above.** A real EURC→TRY quote *mechanism* is demonstrated end-to-end against Ferry's own mock anchor (`mock-anchor/`, rate 44.5 TRY/EURC, 0.5% fee) — but that rate is explicitly illustrative, not a production anchor's economics (`TESTNET_HASHES.md` §8, §5 above) | Seconds for the Stellar leg (SCP ledger close ≈5s); end-to-end speed still bounded by each anchor's own fiat rails, which aren't yet contracted | `TESTNET_HASHES.md` §8 (mock-anchor SEP-38 quote), this document §5 |

**What this table can and can't claim:**
- Wise's row is genuinely live and dated — reproducible by anyone re-running the same calculator today (rates move day to day, so an exact re-fetch will differ slightly).
- Western Union's and the bank-wire row are **estimates built from published, cited fee/markup data**, not live quotes for this exact corridor and amount — stated as such in each cell rather than presented with false precision. Getting genuine live WU and bank quotes requires either a browser automation tool this environment doesn't have, or a human completing each provider's send flow directly (same limitation already documented for the Circle EURC faucet in `TESTNET_HASHES.md` §8.3).
- Ferry's row cannot yet show a competitive real number for the same reason §1–§2 remain templates: no contracted anchor fee schedule exists. What *is* real and demonstrated is the mechanism — a firm, pre-committed, itemized quote shown before the sender commits funds, which none of the three incumbents above do to the same degree (Wise comes closest by disclosing its fee upfront; Western Union and traditional banks embed some or all of their margin in an unstated rate).

---

*This template mirrors the structure `TESTNET_HASHES.md` uses for technical evidence: real, dated, sourced data only. Replace every `[ ]` in §1–§4 before this document is used as SOW evidence of a confirmed anchor relationship — §5's EURC/mock-TRY detail and §7's cost baseline are already real, cited, and current, but neither is a substitute for the anchor relationships and production pricing §1–§4 describe.*
